...
export class VersionManagementComponent implements OnInit {
  ...
  ngOnInit() {
    this.larger_version = true;
    this.currentUserRole = this.commonService.get_user_data();
  ...
  GenerateJson(appId) {
    ...
    this.commonService.postData({
      ...
      if (this.rstatus === 1 || this.rstatus === '1') {
        ...
        let user = this.commonService.get_user_data();
        //when user is admin, admin can get json file for app
        if(user.role_id == 1){
          let jsonUrl = environment.baseUrl + "/projects/" + this.commonService.get_current_app_data().app_code + "/assets/setting.json";
  ...
  generateNewJSON(form: NgForm) {
    ...
    //part compare version up kwai
    let arr_version  = 0;
    if(this.versionManagementData.length != 0){
      for(let i = 0; i < this.versionManagementData.length; i++){
        if(arr_version < parseFloat(this.versionManagementData[i].version))
          arr_version = parseFloat(this.versionManagementData[i].version);
      }
    }

    if(arr_version >= parseFloat(form.value.new_version)){
      this.larger_version = false;
      return;
    }
