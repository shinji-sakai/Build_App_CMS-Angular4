...
export class PushNotificationComponent implements OnInit, CanComponentDeactivate {
  ...
  //gjc
  checked_channel: any[];
  sel_channel_btn: string;
  sel_all_channel: string;
  channel_index: any[];
  channel_strArray: any[];
  ...
  constructor(
    ...
    this.filter_channel = false;
    this.firebase_text = '';

    //gjc 0526
    this.channel_strArray = [-2];
  ...
  ngOnInit() {
    ...
    this.sel_channel_btn = "None selected";
  }
    ...
    return Observable.create((observer) => {
      ...
      const FireBaseBuffer = {
        value: {
          ...
          select_channel: JSON.stringify(this.channel_strArray)
        }
      };
      this.channel_strArray = [];
      this.filter_channel = false;
      debugger;
  ...
  get_channel(){
    const postData = {
        'app_basic_id': this.currentAppData['id']
    }

    this.commonService.postData(postData, 'appmenudata').subscribe(res => {
        let rData = JSON.parse(res);
        let menuData = rData['data'];
        for(let i=0; i < menuData.length; i++){
          if(menuData[i]['menuTypeId'] == 6){
            let notification_menu = JSON.parse(menuData[i]['menu_type_json_data']);
            this.channel_kind = notification_menu['notification_text_fields'];
            this.AfterGetNotifynMenuData();
          }
        }
    });
  }

  AfterGetNotifynMenuData(){
    if(this.channel_kind && this.channel_kind.length != 0)
      this.view_channel = this.channel_kind;
    else{
      this.sel_channel_btn = "Nothing exist"
      return;
    }
  }

  checkChannel(index: number){
    let cntChecked = 0;
    if(this.sel_channel_btn == "None selected"){
      this.sel_channel_btn = "";
    }
    else if(this.sel_channel_btn == "All selected"){
      this.sel_channel_btn = this.sel_all_channel;
    }
    //del channel
    if(this.sel_channel_btn.indexOf(this.view_channel[index]['add_text_field']) != -1){
      this.channel_index = [];
      let strIndex = this.sel_channel_btn.indexOf(this.view_channel[index]['add_text_field']);
      this.sel_channel_btn = this.sel_channel_btn.substring(0, strIndex);
      if(this.sel_channel_btn == ""){
        this.sel_channel_btn = "None selected";
        this.channel_index = [-1];
      }
      else{
        for(let i=0; i < this.view_channel.length; i++){
          if(this.sel_channel_btn.indexOf(this.view_channel[i]['add_text_field']) != -1){
            this.channel_index[cntChecked] = i;
            cntChecked ++;
          }
        }
      }
      ...
      this.channel_index = [];
      this.sel_channel_btn +=  this.view_channel[index]['add_text_field'] + ", ";
      for(let i=0; i < this.view_channel.length; i++){
        if(this.sel_channel_btn.indexOf(this.view_channel[i]['add_text_field']) != -1){
          this.channel_index[cntChecked] = i;
          cntChecked ++;
      ...
      if(cntChecked == this.view_channel.length){
        this.sel_all_channel = this.sel_channel_btn;
        this.sel_channel_btn = "All selected";
        this.channel_index = [-2];
      }
    }
    //send data for push notification channel array
    if(this.channel_index[0] != -2 || this.channel_index[0] != -1){
      for(let i = 0; i < this.channel_index.length; i++){
        this.channel_strArray[i] = this.view_channel[this.channel_index[i]]['add_text_field'];
      }
    }
    else
      this.channel_strArray = this.channel_index[0];
    debugger;
  }
}
