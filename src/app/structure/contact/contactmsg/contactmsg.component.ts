...
export class ContactmsgComponent implements OnInit {
  ...
  onSubmit(form: NgForm){
    if(form.value.msg_content == null || form.value.msg_content == ""){
      $(function () {
        $.notify({
          title: '',
          message: "You should write anything."
        }, {
            type: 'danger'
          });
      });
        ...
        this.commonService.postData(
          {
            ...
            readable  : readable
          },readableUrl).subscribe(res => {
            //to do for update success
        });
        this.setPageMsg(1);
      }
    });
  ...
  calculateTimeMsg(modifyTime: any){
    ...
    var curHour   = currentDate.getUTCHours();//parseInt(currentDate.toString().substring(16,18));
    var curMin    = currentDate.getUTCMinutes();//parseInt(currentDate.toString().substring(19,21));
    let strAgo;
    //year
