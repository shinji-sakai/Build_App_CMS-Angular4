...
import { AboveNumValidator } from './../../directive/above-num.validator';
// import { AboveNumValidatorDirective } from './../../directive/above-num.directive';
...
export class AddBonusComponent implements OnInit {
	...
	ngOnInit() {
		...
		this.form = this.fb.group({
			...
			description: '',
			cost: [0, Validators.compose([AboveNumValidator(0)])],
			image: ''
		});
		$('.dropify').dropify({
			messages: {
	...
	onSubmit(form: NgForm) {
		...
		this.fileUploadObservable().subscribe((res) => {
			...
			this.commonService.postData(form.value, 'addBonus').subscribe(res => {
					this.router.navigate(['/rewards/index']);
