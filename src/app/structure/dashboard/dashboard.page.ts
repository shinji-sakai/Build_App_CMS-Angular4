...
export class Dashboard implements OnInit {
  ...
  appListing(paginum: number, active: boolean) {
      if(this.pager.currentPage == 1 && this.pager.currentPage == this.pager.totalPages && active == true){
        return;
      }
      NProgress.start();
  ...
  navigateApp(data) {
    ...
    this.commonService.postData(postData, 'getsingleappdata').subscribe(res => {
      ...
      this.commonService.postData({
        'app_basic_id': appData['id']
      }, 'appmenudata').subscribe(res => {
        let rData = JSON.parse(res);
        let menuData = rData['data'];
        if (menuData.length !== 0) {
          this.msgService.setMenuListData(menuData);
        }
    });
